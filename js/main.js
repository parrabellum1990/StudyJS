...
// Список комбинаций:
var uaprName = [' Вольт ', ' Ампер ', ' Ом ', ' Ватт '];
...
$(function () {
  $('#resultCalculate').click(function (e) {
    ...
    if (statusParametr == uprName[1]) {
      ...
      $('#resultTotal').html($('#resultTotal').text() + resultTotalRootMultiplication.toFixed(6) + uaprName[0]);
    } else {
      if (statusParametr == uarName[1]) {
        resultTotalMultiplication;
        $('#resultTotal').html($('#resultTotal').text() + resultTotalMultiplication.toFixed(6) + uaprName[0]);
      } else {
        if (statusParametr == upaName[1]) {
          resultTotalSegmentation;
          $('#resultTotal').html($('#resultTotal').text() + resultTotalSegmentationYX.toFixed(6) + uaprName[0]);
        }
      }
    }
    // По току:
    if (statusParametr == aurName[1]) {
      resultTotalSegmentation;
      $('#resultTotal').html($('#resultTotal').text() + resultTotalSegmentation.toFixed(6) + uaprName[1]);
    } else {
      if (statusParametr == apuName[1]) {
        ...
        if (statusParametr == aprName[1]) {
          ...
          $('#resultTotal').html($('#resultTotal').text() + resultTotalRootSegmentationYX.toFixed(6) + uaprName[1]);
        }
      }
    }
    // По сопротивлению:
    if (statusParametr == rupName[1]) {
      ...
      $('#resultTotal').html($('#resultTotal').text() + resultTotalSegmentationRootNumerator.toFixed(6) + uaprName[2]);
    } else {
      if (statusParametr == ruaName[1]) {
        resultTotalSegmentationYX;
        $('#resultTotal').html($('#resultTotal').text() + resultTotalSegmentationYX.toFixed(6) + uaprName[2]);
      } else {
        if (statusParametr == rpaName[1]) {
          resultTotalSegmentationRootDenominator;
          $('#resultTotal').html($('#resultTotal').text() + resultTotalSegmentationRootDenominator.toFixed(6) + uaprName[2]);
        }
      }
    }
    // По мощности:
    if (statusParametr == purName[1]) {
      ...
      $('#resultTotal').html($('#resultTotal').text() + resultTotalSegmentationRootNumeratorYX.toFixed(6) + uaprName[3]);
    } else {
      if (statusParametr == parName[1]) {
        resultTotalMultiplicationRootNumeratorYX;
        $('#resultTotal').html($('#resultTotal').text() + resultTotalMultiplicationRootNumeratorYX.toFixed(6) + uaprName[3]);
      } else {
        if (statusParametr == puaName[1]) {
          resultTotalMultiplication;
          $('#resultTotal').html($('#resultTotal').text() + resultTotalMultiplication.toFixed(6) + uaprName[3]);
        }
      }
    }
    if (parametrX[0] == '-') {
